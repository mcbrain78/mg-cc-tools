...
**Resolution logic:**
1. If `$ARGUMENTS` is empty, use the current project root (parent of `.claude/` where this command is installed). Derive from `{SOURCE_DIR}`: go up one level from the source directory to get the mg-cc-tools root, that's the target.
2. If `$ARGUMENTS` starts with `/`, use it as-is → `TARGET_PROJECT=$ARGUMENTS`
3. Otherwise, resolve as sibling: find the mg-cc-tools root from `{SOURCE_DIR}` (go up one level), then go up one more level, and append the project name → `TARGET_PROJECT=<parent>/<name>`
...
cp "{SOURCE_DIR}/hooks/permission-guard.py" "<TARGET_HOOKS_DIR>/permission-guard.py"
...
SOURCE_MD5=$(grep -v '^PROJECT_ROOT = ' "{SOURCE_DIR}/hooks/permission-guard.py" | md5sum | cut -d' ' -f1)
