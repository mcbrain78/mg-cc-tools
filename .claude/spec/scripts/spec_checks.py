...
Everything the autonomous loop must NOT eyeball in-context — vote tallies,
the block-gate threshold, template-structure checks, the deterministic floor,
decision-record rendering and status projection, and the atom ledger. The
orchestrator and its agents invoke these via Bash and read the raw JSON stdout;
they branch only on values a script computed. stdlib plus a subprocess call
into the sibling ``milestone_checks.py`` (floor only).
...
def cmd_atoms_churn_check(ledger_path: Path) -> int:
    ...
    that many times, so the caller does only set-membership lookups."""
    try:
        ledger = _load_ledger(ledger_path)
    except (ValueError, json.JSONDecodeError) as e:
        return _fail(f"cannot read ledger: {e}")
