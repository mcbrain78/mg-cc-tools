...
3. **Set findings paths.**
   - `findings_dir = .mg/docs/auditv2/run`
   - `suppress_file = .mg/docs/auditv2/suppressed-findings.json`

4. **Check xml-sources exist.** Use Glob to verify `.mg/docs/generate/xml-sources/` contains `.xml` files. If not, abort with:
   ```
...
```bash
uv run .claude/auto-doc/scripts/load-audit-findings.py \
...
Read and follow the instructions in: .claude/auto-doc/agents/group-findings.md
...
1. Archive the previous fix run if it exists (has `fix-state.json`), then create a fresh fix directory:
   ```bash
   if [ -f .mg/docs/fix/fix-state.json ]; then
     NEXT_NUM=$(python3 -c "
   import os, re
   ...
   ")
     ...
     mv .mg/docs/fix \
        .mg/docs/auditv2/history/fix-${NEXT_NUM}
   fi
   mkdir -p .mg/docs/fix
   ```
...
3. Initialize the fix queue:
   ```bash
   uv run .claude/auto-doc/scripts/fix-queue.py init \
       ...
       --edit-dir .mg/docs/fix \
       --suppress-file {suppress_file} \
       --approved {comma_separated_indices} \
       --state-file .mg/docs/fix/fix-state.json
   ```
...
4. **Loop** — call `next` repeatedly until done:
   ...
   a. Get the next group:
      ```bash
      uv run .claude/auto-doc/scripts/fix-queue.py next \
          --state-file .mg/docs/fix/fix-state.json
      ```
   ...
   d. If `status` is `"next"`: spawn a **single foreground** agent to edit the group's file. Pass both `edit_file` and `suppress_file` from the `next` output into the prompt:
      ...
      Read and follow the instructions in: .claude/auto-doc/agents/audit-fixer.md
...
Run `/mg:transcript-export md-subagent .mg/docs/fix/transcript.md`
