...
```bash
uv run .claude/auto-doc/scripts/generate-setup.py \
  --scan-file .mg/docs/docs-scan.json \
  --config .mg/docs/.docs.config.json \
  --global-config .claude/auto-doc/references/.docs.config.json \
...
1. **Spawn a single Agent call** with the glossary-writer agent:
   ...
   Read and follow the instructions in: .claude/auto-doc/agents/glossary-writer.md

   Project root: {project_root}
   Docs dir: {docs_dir_abs}
   Scan data path: {scan_views.glossary}
   Project model path: {project_model_path}
   Glossary template path: .claude/auto-doc/references/templates/GLOSSARY.template.md
   Style guide path: .claude/auto-doc/references/style-guide.md
   Mode: {mode}
   Pass: initial
...
2. **After agent completes, finalize the glossary state.** Check that the state file exists before running:
   ...
   ```bash
   uv run .claude/auto-doc/scripts/write-section.py \
       --finalize \
       --state-file .mg/docs/generate/write-state-glossary.json \
       --docs-dir {docs_dir_abs} \
       --audience "" \
       --manifest-file .mg/docs/generate/manifest-glossary.json \
       --mode {mode} \
...
3. **Assemble markdown.** If `.mg/docs/generate/xml-sources/GLOSSARY.xml` exists:

   ```bash
   uv run .claude/auto-doc/scripts/assemble-markdown.py \
       --xml-file .mg/docs/generate/xml-sources/GLOSSARY.xml \
...
2. **Spawn one Agent call per (audience, document) pair** in `audiences` (from setup output) in a SINGLE message (parallel foreground -- do NOT set `run_in_background`). Each subagent reads its own instructions.
   ...
   **Orient-write routing (all audiences).** For each `(audience, document)` pair, check `refined_templates[audience][DOCUMENT]`. If the entry is non-null, spawn a dedicated orient-write Agent for that document. If the entry is null, use the standard prompt for that document.
   ...
   Read and follow the instructions in: .claude/auto-doc/agents/{audience_writer_file}
   ...
   Scan data path: {scan_views[audience]}
   Project model path: {project_model_path}
   Templates dir: .claude/auto-doc/references/templates/{audience}/
   Style guide path: .claude/auto-doc/references/style-guide.md
   Glossary path: {docs_dir_abs}/GLOSSARY.md
...
For each `write-state-{audience}-{DOCUMENT}.json` file in `.mg/docs/generate`:

```bash
uv run .claude/auto-doc/scripts/write-section.py \
    --finalize \
    --state-file .mg/docs/generate/write-state-{audience}-{DOCUMENT}.json \
    --docs-dir {docs_dir_abs} \
    --audience {audience} \
    --manifest-file .mg/docs/generate/manifest-{audience}.json \
    --mode {mode} \
    --xml-dir .mg/docs/generate/xml-sources
```
...
uv run .claude/auto-doc/scripts/write-section.py \
    --finalize \
    --state-file .mg/docs/generate/write-state-{audience}.json \
    --docs-dir {docs_dir_abs} \
    --audience {audience} \
    --manifest-file .mg/docs/generate/manifest-{audience}.json \
    --mode {mode} \
...
```bash
uv run .claude/auto-doc/scripts/assemble-markdown.py \
    --xml-file .mg/docs/generate/xml-sources/{audience}/{DOCUMENT}.xml \
...
Read and follow the instructions in: .claude/auto-doc/agents/doc-polisher.md
...
```bash
uv run .claude/auto-doc/scripts/sync-edits-to-xml.py \
...
```bash
uv run .claude/auto-doc/scripts/merge-manifests.py \
    --tmp-dir .mg/docs/generate \
    --output-dir .mg/docs/generate/reference-manifests \
...
3. **Reassemble.** If `.mg/docs/generate/xml-sources/GLOSSARY.xml` exists:
...
1. **Prepare review chunks** from the generated docs:
   ```bash
   uv run .claude/auto-doc/scripts/prepare-doc-review.py \
       --docs-dir {docs_dir_abs} \
       --output-dir .mg/docs/generate/overview-chunks \
       --token-limit 5000
   ```
...
2. **Spawn the overview writer subagent:**
   ...
   Agent(
     ...
     prompt="Read and follow: .claude/auto-doc/agents/overview-writer.md

     Project root: {project_root}
     Docs dir: {docs_dir_abs}
     Review manifest: .mg/docs/generate/overview-chunks/manifest.json
     Project model path: .mg/docs/generate/project-model.json
     Glossary path: {docs_dir_abs}/GLOSSARY.md
     OVERVIEW template: .claude/auto-doc/references/templates/OVERVIEW.template.md
     Style guide path: .claude/auto-doc/references/style-guide.md
     Tmp dir: {generate_dir}
     Scripts dir: .claude/auto-doc/scripts"
   )
...
3. **After agent completes, finalize the overview state.** Check that the state file exists before running:
   ...
   uv run .claude/auto-doc/scripts/write-section.py \
       --finalize \
       --state-file .mg/docs/generate/write-state-overview.json \
       --docs-dir {docs_dir_abs} \
       --audience "" \
       --manifest-file .mg/docs/generate/manifest-overview.json \
       --mode {mode} \
       --xml-dir .mg/docs/generate/xml-sources
   ```
...
4. **Reassemble.** If `.mg/docs/generate/xml-sources/OVERVIEW.xml` exists:

   ```bash
   uv run .claude/auto-doc/scripts/assemble-markdown.py \
       --xml-file .mg/docs/generate/xml-sources/OVERVIEW.xml \
...
- **Directory layout, source material key format, and data contracts** are documented in `.claude/auto-doc/references/schema.yaml`. Workspace setup (directories, artifact cleanup, scan splitting) is handled by `generate-setup.py`.
