...
- `mg:spec-prepare-context` splits the concept content into per-phase files
- `mg:spec-create-context` transforms those into CONTEXT.md (locked decisions)
...
Usage: /mg:spec-gsd-phases <source-file-path>

Examples:
  /mg:spec-gsd-phases docs/work-queue/todo/recursive-section-xml/concept.md
  /mg:spec-gsd-phases docs/design/auth-overhaul.md
```
Exit.
...
Split the concept into per-phase files and create CONTEXT.md for each phase:
  ...
  /mg:spec-prepare-context {first_phase}-{last_phase} {source-file-path}
