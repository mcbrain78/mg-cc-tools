...
"Read the file .claude/mg-gsd-wrappers/references/requirement-generator.md and follow its instructions using {context_path} as the context file."
...
Replace `{context_path}` with the actual CONTEXT.md path before spawning. The path `.claude/mg-gsd-wrappers/references/requirement-generator.md` is resolved at install time by install.sh
