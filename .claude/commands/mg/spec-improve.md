...
Also read the concept spec template at: .claude/spec/references/concept-spec-template.md
...
- **Fixes**:
  - `approve` → run: `uv run .claude/spec/scripts/improve_files.py approve <target-file-path>`
  - `reject` → run: `uv run .claude/spec/scripts/improve_files.py reject <target-file-path>`
...
- **Non-goals**: For each proposed non-goal the user approves, run:
  ```
  uv run .claude/spec/scripts/improve_files.py append-non-goal <target-file-path> "<non-goal text>"
  ```
