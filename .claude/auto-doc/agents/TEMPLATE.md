...
- **templates_dir**: Path to `.claude/auto-doc/references/templates` -- the audience-specific subdirectory for this agent (e.g., `.claude/auto-doc/references/templates/end-users/`).
- **style_guide_path**: Path to `.claude/auto-doc/references/style-guide.md`.
...
3. **Propose new terms** -- For any domain-specific or technical terms used in the generated content that are not already in the glossary, output a JSON array of term proposals:
   ...
   ```
   Write proposals to `.mg/docs/generate/terms/terms-{audience}.json`.
...
2. **Four writer agents run in parallel** -- One agent per audience (end-user, developer, agent, devops). Each reads its templates and generates all assigned documents. Agents reference the glossary for consistent terminology. Each agent writes term proposals to `.mg/docs/generate/terms/terms-{audience}.json`.
