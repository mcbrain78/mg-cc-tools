...
- **glossary_template_path**: Path to `.claude/auto-doc/references/templates/GLOSSARY.template.md`.
...
- **term_proposals_dir**: Path to `.mg/docs/generate/terms/` where `terms-*.json` files are written by writer agents.
...
5. **Write output via write-section.py** -- Use the section-write workflow instead of writing GLOSSARY.md directly.
   ...
   Write to `.mg/docs/generate/header-glossary-GLOSSARY.md`:
   ```
   ...
   **Step 1: Emit the `##` intro.** Write the `## ` heading line plus the body content up to the first `###` heading (or end of section if no `###` exists).
   ...
   a. Write intro content to `.mg/docs/generate/section-glossary-GLOSSARY-{slug}.md`.
   b. Write refs to `.mg/docs/generate/refs-glossary-GLOSSARY-{slug}.json` with ONLY the typed_refs for entities in the intro body.
      For terms that reference specific code entities, emit typed_refs following the format in: .claude/auto-doc/references/typed-refs-format.md. Use `{"typed_refs": []}` for purely conceptual terms.
   c. Call:
      ```bash
      uv run .claude/auto-doc/scripts/write-section.py \
          ...
          --document GLOSSARY \
          --section {slug} \
          --content-file .mg/docs/generate/section-glossary-GLOSSARY-{slug}.md \
          --refs-file .mg/docs/generate/refs-glossary-GLOSSARY-{slug}.json \
          --header-file .mg/docs/generate/header-glossary-GLOSSARY.md
      ```
   ...
   **Step 2: Emit each `###` child** (if any). Current glossary templates use `##` sections only. `###` headings are rare but supported. For each `###` heading within this `##` section:
   ...
   a. Write content to `.mg/docs/generate/section-glossary-GLOSSARY-{slug}-{child-slug}.md`.
   b. Write refs to `.mg/docs/generate/refs-glossary-GLOSSARY-{slug}-{child-slug}.json` with ONLY the typed_refs for entities in this `###` body.
   c. Call:
      ```bash
      uv run .claude/auto-doc/scripts/write-section.py \
          --state-file .mg/docs/generate/write-state-glossary.json \
          ...
          --section {child-slug} \
          --parent {slug} \
          --content-file .mg/docs/generate/section-glossary-GLOSSARY-{slug}-{child-slug}.md \
          --refs-file .mg/docs/generate/refs-glossary-GLOSSARY-{slug}-{child-slug}.json
      ```
   ...
   **Completeness:** If a glossary definition names a function, class, table, or config file in backticks, emit a matching typed ref. For `db` refs, always include the `db` field (database name). If a term is purely conceptual (no specific code entity named), use `{"typed_refs": []}`. Do NOT attach refs for code entities the definition doesn't name. See the Completeness Rule, Contextual Ref Patterns, and Self-check in `.claude/auto-doc/references/typed-refs-format.md` — especially the column-level db refs pattern (each metric definition in `domain-terms` typically needs its own `db column` ref) and the framework-symbol dep pattern (one `dep` ref per framework, not per usage).
...
5. **Write reconciliation log** -- Write a summary to `.mg/docs/generate/terms/glossary-reconciliation.log` documenting:
...
- **Follow the style guide.** Use active voice, present tense, and concrete language per `.claude/auto-doc/references/style-guide.md`.
