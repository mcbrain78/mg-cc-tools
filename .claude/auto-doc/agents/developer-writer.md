...
- **generate_dir**: Path to the generate workspace directory (e.g., `.mg/docs/generate`).
...
2. **For each assigned document:**
   ...
   a. **Write the document header** to `.mg/docs/generate/header-developers-{DOCUMENT}.md`:
      ```
   ...
   b. **First call to next-heading.py:**
      ```bash
      uv run .claude/auto-doc/scripts/next-heading.py \
   ...
   c. **LOOP** until done:
      ...
      - **If `type` = `"write"`:**
        ...
        **Generate content** for this heading:
        ...
        Write content to `.mg/docs/generate/section-developers-{DOCUMENT}-{heading_path_dashed}.md` (replace `/` with `-` in `heading_path` for the filename).

        Write typed_refs to `.mg/docs/generate/refs-developers-{DOCUMENT}-{heading_path_dashed}.json` with ONLY the refs for entities mentioned in this heading's content.
        ...
        **Do NOT write heading lines** (`##`, `###`, `####`) — they are injected automatically by `write-section.py` via `--heading-state`.
        ...
        uv run .claude/auto-doc/scripts/write-section.py \
          --state-file .mg/docs/generate/write-state-developers-{DOCUMENT}.json \
          --document {DOCUMENT} \
          --section {section_slug} \
          [--parent {parent_path}] \
          --content-file .mg/docs/generate/section-developers-{DOCUMENT}-{heading_path_dashed}.md \
          --refs-file .mg/docs/generate/refs-developers-{DOCUMENT}-{heading_path_dashed}.json \
          [--header-file .mg/docs/generate/header-developers-{DOCUMENT}.md] \
...
3. **Propose new terms** — For technical terms used in the generated content that are not already in the glossary, output a JSON array of term proposals:
   ...
   ```
   Write proposals to `.mg/docs/generate/terms/terms-developers-{DOCUMENT}.json`.
...
**Code-block completeness.** Follow the Completeness Rule, Contextual Ref Patterns, and Self-check in `.claude/auto-doc/references/typed-refs-format.md`. The `db` field in db refs comes from `database_name` in the orient response when present.

Read and follow the typed refs format in: .claude/auto-doc/references/typed-refs-format.md
