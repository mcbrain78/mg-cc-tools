...
- **templates_dir**: Path to `.claude/auto-doc/references/templates` -- the base templates directory containing audience subdirectories and shared templates.
...
5. **Write output.** Write the complete scan output JSON to a temp file, then call the validation script to write it atomically to `output_path`:
   ...
   a. Write the JSON to a temp file via the Write tool (e.g., `.mg/docs/scan/scan-{audience}.json`). The JSON structure is the same as the Output Format below.

   b. Call the validation script with `--sections-file` for each parsed template:
      ```bash
      uv run .claude/auto-doc/scripts/write-scan-output.py \
        --input .mg/docs/scan/scan-{audience}.json \
        --output {output_path} \
        --audience {audience} \
