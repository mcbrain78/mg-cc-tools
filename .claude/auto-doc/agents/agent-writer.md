...
2. **For each assigned document:**
   ...
   a. **Write the document header** to `.mg/docs/generate/header-agents-{DOCUMENT}.md`:
      ```
      ---
      tool: {product_name}
   ...
   c. **LOOP** until done:
      ...
      - **If `type` = `"write"`:**
        ...
        **Generate content** for this heading:
        ...
        Write content to `.mg/docs/generate/section-agents-{DOCUMENT}-{heading_path_dashed}.md`.

        Write typed_refs to `.mg/docs/generate/refs-agents-{DOCUMENT}-{heading_path_dashed}.json`.
        ...
        **Do NOT write heading lines** — they are injected automatically by `write-section.py` via `--heading-state`.
        ...
        uv run .claude/auto-doc/scripts/write-section.py \
          --state-file .mg/docs/generate/write-state-agents-{DOCUMENT}.json \
          --document {DOCUMENT} \
          --section {section_slug} \
          [--parent {parent_path}] \
          --content-file .mg/docs/generate/section-agents-{DOCUMENT}-{heading_path_dashed}.md \
          --refs-file .mg/docs/generate/refs-agents-{DOCUMENT}-{heading_path_dashed}.json \
          [--header-file .mg/docs/generate/header-agents-{DOCUMENT}.md] \
...
3. **Propose new terms** — For technical terms used in the generated content not already in the glossary, output a JSON array:
   ...
   ```
   Write to `.mg/docs/generate/terms/terms-agents-{DOCUMENT}.json`.
