...
- **state_file_path**: Path to the `next-heading.py` state file for this document, scoped per document (e.g., `{MG_INSTALL_TMP_DIR}/heading-state-devops-OPERATIONS.json`). Do not read this file directly — `next-heading.py` creates and manages it on first call.
...
2. **For each assigned document:**
   ...
   a. **Write the document header** to `{MG_INSTALL_TMP_DIR}/header-devops-{DOCUMENT}.md`:
      ```
   ...
   c. **LOOP** until done:
      ...
      - **If `type` = `"write"`:**
        ...
        **heading_path splitting examples:**
        ...
        Write content to `{MG_INSTALL_TMP_DIR}/section-devops-{DOCUMENT}-{heading_path_dashed}.md` (replace `/` with `-` in `heading_path` for the filename).

        Write typed_refs to `{MG_INSTALL_TMP_DIR}/refs-devops-{DOCUMENT}-{heading_path_dashed}.json` with ONLY the refs for entities mentioned in this heading's content.
        ...
        **Do NOT write heading lines** (`##`, `###`, `####`) in your content -- they are injected automatically by `write-section.py` via `--heading-state`.
        ...
        python3 {MG_INSTALL_SCRIPTS_DIR}/write-section.py \
          --state-file {MG_INSTALL_TMP_DIR}/write-state-devops-{DOCUMENT}.json \
          --document {DOCUMENT} \
          --section {section_slug} \
          [--parent {parent_path}] \
          --content-file {MG_INSTALL_TMP_DIR}/section-devops-{DOCUMENT}-{heading_path_dashed}.md \
          --refs-file {MG_INSTALL_TMP_DIR}/refs-devops-{DOCUMENT}-{heading_path_dashed}.json \
          [--header-file {MG_INSTALL_TMP_DIR}/header-devops-{DOCUMENT}.md] \
