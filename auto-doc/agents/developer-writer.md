...
2. **For each assigned document:**
   ...
   g. **Write sections and references.** For each section you generated, emit it
      ...
      Write to `{TMP_DIR}/header-developers-{DOCUMENT}.md`:
      ```
      ...
      1. Write section content to `{TMP_DIR}/section-developers-{DOCUMENT}-{section-slug}.md`
      ...
      3. Call:
         ...
         python3 {SCRIPTS_DIR}/write-section.py \
           --state-file {TMP_DIR}/write-state-developers.json \
           --document {DOCUMENT} \
           --section {section-slug} \
           --content-file {TMP_DIR}/section-developers-{DOCUMENT}-{section-slug}.md \
           --refs-file {TMP_DIR}/refs-developers-{DOCUMENT}-{section-slug}.json \
           --header-file {TMP_DIR}/header-developers-{DOCUMENT}.md \
   ...
   h. **Verify section references.** For each section with non-empty `symbols` or
      ...
      printf '%s\n\nContent file: %s\nRefs file: %s' \
        ...
        "{TMP_DIR}/section-developers-{DOCUMENT}-{section-slug}.md" \
        "{TMP_DIR}/refs-developers-{DOCUMENT}-{section-slug}.json" \
