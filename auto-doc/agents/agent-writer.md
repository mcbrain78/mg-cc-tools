...
2. **For each assigned document:**
   ...
   g. **Write sections and references.** For each section you generated, emit it
      ...
      Write to `{TMP_DIR}/header-agents-{DOCUMENT}.md`:
      ```
      ...
      1. Write section content to `{TMP_DIR}/section-agents-{DOCUMENT}-{section-slug}.md`
         (include the `## Heading`, `<!-- docs-meta: ... -->` comment, and all body content)
      2. Write references to `{TMP_DIR}/refs-agents-{DOCUMENT}-{section-slug}.json`:
         ```json
