...
3. **For each assigned document:**
   ...
   e. **Write sections and references.** For each section you generated, emit it
      ...
      Write to `{TMP_DIR}/header-end-users-{DOCUMENT}.md`:
      ```
      ...
      1. Write section content to `{TMP_DIR}/section-end-users-{DOCUMENT}-{section-slug}.md`
         (include the `## Heading`, `<!-- docs-meta: ... -->` comment, and all body content)
      2. Write references to `{TMP_DIR}/refs-end-users-{DOCUMENT}-{section-slug}.json`:
         ```json
      ...
      3. Call:
         ...
         python3 {SCRIPTS_DIR}/write-section.py \
           --state-file {TMP_DIR}/write-state-end-users.json \
           --document {DOCUMENT} \
           ...
           --header-file {TMP_DIR}/header-end-users-{DOCUMENT}.md \
