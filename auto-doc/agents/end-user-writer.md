...
3. **For each assigned document:**
   ...
   e. **Write sections and references.** For each section you generated, emit it
      ...
      Write to `{MG_INSTALL_TMP_DIR}/header-end-users-{DOCUMENT}.md`:
      ```
      ...
      1. Write section content to `{MG_INSTALL_TMP_DIR}/section-end-users-{DOCUMENT}-{section-slug}.md`
         (include the `## Heading`, `<!-- docs-meta: ... -->` comment, and all body content)
      2. Write references to `{MG_INSTALL_TMP_DIR}/refs-end-users-{DOCUMENT}-{section-slug}.json`:
         ```json
      ...
      3. Call:
         ...
         python3 {MG_INSTALL_SCRIPTS_DIR}/write-section.py \
           --state-file {MG_INSTALL_TMP_DIR}/write-state-end-users.json \
           --document {DOCUMENT} \
           --section {section-slug} \
           --content-file {MG_INSTALL_TMP_DIR}/section-end-users-{DOCUMENT}-{section-slug}.md \
           --refs-file {MG_INSTALL_TMP_DIR}/refs-end-users-{DOCUMENT}-{section-slug}.json \
           --header-file {MG_INSTALL_TMP_DIR}/header-end-users-{DOCUMENT}.md \
   ...
   f. **Verify section references.** For each section, run the verification script:
      ...
      python3 {MG_INSTALL_SCRIPTS_DIR}/verify-section-refs.py \
        ...
        --log-file {MG_INSTALL_TMP_DIR}/verification-log.json
      ```
