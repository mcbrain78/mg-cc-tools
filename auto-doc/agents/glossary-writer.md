...
5. **Write output via write-section.py** -- Use the section-write workflow instead of writing GLOSSARY.md directly.
   ...
   **Completeness:** If a glossary definition names a function, class, table, or config file in backticks, emit a matching typed ref. For `db` refs, always include the `db` field (database name). If a term is purely conceptual (no specific code entity named), use `{"typed_refs": []}`. Do NOT attach refs for code entities the definition doesn't name. For `enum` refs, the `class` must be a Python `Enum` subclass or `Literal[...]` type alias (e.g., `DriftSeverity`) — never a SQLAlchemy table model. See Completeness Rule in typed-refs-format.md.
