...
"Read the file {MG_INSTALL_GENERATOR_PROMPT} and follow its instructions using {context_path} as the context file."
...
Replace `{context_path}` with the actual CONTEXT.md path before spawning. The path `{MG_INSTALL_GENERATOR_PROMPT}` is resolved at install time by install.sh
