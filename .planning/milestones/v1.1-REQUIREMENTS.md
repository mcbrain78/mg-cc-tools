...
- [~] **CMD-01**: `/mg:create-docs` router detects pipeline state (no docs, existing docs, partial scan) and routes to correct step — *Removed in v1.1 stabilization (90fa640): router didn't add value, users invoke step commands directly*
...
- [~] **CMD-05**: `/mg:add-docs` captures a note to inbox with auto-classification (audience, document, section, confidence) — *Removed in v1.1 stabilization (90fa640): notes subsystem excised*
...
- [~] **DOC-04**: End-user domain-specific documents defined in project config — *Superseded by template-refiner (Phase 23 / TRF-01..07): projects add or edit refined templates at `.mg/docs/templates/end-users/*.template.md` directly; no separate `custom_documents` config mechanism needed*
...
- [~] **SCN-06**: Notes inbox classification: pending notes assigned to audience, document, section — *Removed in v1.1 stabilization (90fa640): notes subsystem excised*
...
- [~] **GEN-03**: Notes integration: approved inbox notes expanded and placed at classified locations — *Removed in v1.1 stabilization (90fa640): notes subsystem excised*
...
- [~] **VFY-03**: Diataxis mixing detection: flag tutorial content in reference docs, explanation in how-to — *Removed in v1.1 stabilization (90fa640): named verify checks dropped*
- [~] **VFY-04**: Completeness: major code components have documentation for each relevant audience — *Removed in v1.1 stabilization (90fa640): named verify checks dropped*
- [~] **VFY-05**: Example validity: code examples are syntactically valid — *Removed in v1.1 stabilization (90fa640): named verify checks dropped*
- [~] **VFY-06**: Link integrity: internal markdown links between docs resolve — *Removed in v1.1 stabilization (90fa640): named verify checks dropped*
- [~] **VFY-07**: Output as docs-verify-report.md with issues by severity — *Removed in v1.1 stabilization (90fa640): named verify checks dropped; severity removed in d6ad5a0*
...
- [~] **SCRIPT-01**: `/mg:auto-doc-script` command file exists with correct frontmatter (name, description, allowed-tools: Bash, Read, Write, Glob, Grep) and implements argument parsing, validation, single-file mode, directory mode, and README generation — *Removed in v1.1 stabilization (90fa640): standalone script-doc command dropped*
- [~] **SCRIPT-02**: `SCRIPT_README.template.md` template exists with `<!-- PURPOSE -->`, `<!-- EXAMPLE -->`, and `<!-- OPTIONAL -->` annotations for all sections (title/description, Prerequisites, Usage, Examples, Options, Output, Notes) — *Removed in v1.1 stabilization (90fa640): template now orphaned*
- [~] **SCRIPT-03**: `auto-doc-script` added to COMMANDS array in `auto-doc/install.sh`, deploys correctly with `{TEMPLATES_DIR}` sed resolution — *Removed in v1.1 stabilization (90fa640)*
- [~] **SCRIPT-04**: Default output filename is `README.AUTO-DOC.md`, placed in same directory as target file or inside target directory — *Removed in v1.1 stabilization (90fa640)*
- [~] **SCRIPT-05**: Directory mode warns at >20 source files, errors on zero source files, handles multiple entry points with summary table and per-script sections — *Removed in v1.1 stabilization (90fa640)*
- [~] **SCRIPT-06**: Generated examples use ` ```console ` fenced code blocks with `$` prompt prefix and expected output shown — *Removed in v1.1 stabilization (90fa640)*
...
## Removed and Superseded in v1.1

Requirements that are no longer in active v1.1 scope, either because they were intentionally removed during stabilization or because a later architecture made them obsolete. The REQ-IDs are kept here for traceability — checkboxes marked `[~]`, traceability table shows status `Removed` or `Superseded`, active v1 coverage count excludes them.

### Removed in v1.1 Stabilization (15 reqs, commit `90fa640` 2026-04-30)

User confirmed these as intentional scope reductions on 2026-05-04. Commit `90fa640` pruned auto-doc to its final 5-command pipeline (scan → prepare-templates → generate → auditv2 → fix), removing 8 commands, 12 agents, 15 scripts.

| REQ-ID | Original Phase | Reason |
|--------|----------------|--------|
| CMD-01 | Phase 5 | Router didn't add value; users invoke step commands directly |
| CMD-05 | Phase 5 | Notes subsystem excised |
| SCN-06 | Phase 3 | Notes subsystem excised |
| GEN-03 | Phase 4 | Notes subsystem excised |
| VFY-03 | Phase 5 | Diataxis mixing check dropped |
| VFY-04 | Phase 5 | Completeness check dropped |
| VFY-05 | Phase 5 | Example validity check dropped |
| VFY-06 | Phase 5 | Link integrity check dropped |
| VFY-07 | Phase 5 | Severity-categorized output dropped (severity removed in d6ad5a0) |
| SCRIPT-01 | Phase 13 | `/mg:auto-doc-script` standalone command dropped |
| SCRIPT-02 | Phase 13 | SCRIPT_README template now orphaned (kept in references/templates/) |
| SCRIPT-03 | Phase 13 | install.sh COMMANDS array entry removed |
| SCRIPT-04 | Phase 13 | Standalone command dropped |
| SCRIPT-05 | Phase 13 | Standalone command dropped |
| SCRIPT-06 | Phase 13 | Standalone command dropped |

### Superseded by Architecture Evolution (1 req)

| REQ-ID | Original Phase | Superseded By | Reason |
|--------|----------------|---------------|--------|
| DOC-04 | Phase 4 | Phase 23 (TRF-01..07) | Template-refiner makes the `custom_documents` config mechanism obsolete: projects add or edit refined templates at `.mg/docs/templates/end-users/*.template.md` directly |

**Total removed:** 15 requirements
**Total superseded:** 1 requirement
**Combined out-of-active-scope:** 16 requirements

**Implementation artifacts still present** (cleanup candidates):
- `auto-doc/references/templates/SCRIPT_README.template.md` — orphaned template, no command invokes it
- `auto-doc/commands/auto-doc-auditv2.md` contains misleading phrase "That's verify's job" implying a verify command still exists
...
|-------------|-------|--------|
| CMD-01 | Phase 5 | Removed (90fa640) |
| CMD-02 | Phase 3 | Complete |
| CMD-03 | Phase 4 | Complete |
| CMD-04 | Phase 5 | Complete |
| CMD-05 | Phase 5 | Removed (90fa640) |
| DOC-01 | Phase 4 | Complete |
| DOC-02 | Phase 4 | Complete |
| DOC-03 | Phase 4 | Complete |
| DOC-04 | Phase 4 | Superseded by Phase 23 |
...
| SCN-06 | Phase 3 | Removed (90fa640) |
| SCN-07 | Phase 3 | Complete |
| SCN-08 | Phase 3 | Complete |
| GEN-01 | Phase 4 | Complete |
| GEN-02 | Phase 4 | Complete |
| GEN-03 | Phase 4 | Removed (90fa640) |
| GEN-04 | Phase 4 | Complete |
| GEN-05 | Phase 4 | Complete |
| GEN-06 | Phase 4 | Complete |
| VFY-01 | Phase 5 | Complete |
| VFY-02 | Phase 5 | Complete |
| VFY-03 | Phase 5 | Removed (90fa640) |
| VFY-04 | Phase 5 | Removed (90fa640) |
| VFY-05 | Phase 5 | Removed (90fa640) |
| VFY-06 | Phase 5 | Removed (90fa640) |
| VFY-07 | Phase 5 | Removed (90fa640) |
...
| SCRIPT-01 | Phase 13 | Removed (90fa640) |
| SCRIPT-02 | Phase 13 | Removed (90fa640) |
| SCRIPT-03 | Phase 13 | Removed (90fa640) |
| SCRIPT-04 | Phase 13 | Removed (90fa640) |
| SCRIPT-05 | Phase 13 | Removed (90fa640) |
| SCRIPT-06 | Phase 13 | Removed (90fa640) |
...
**Coverage:**
- v1 requirements: 234 total (218 active + 15 removed + 1 superseded)
- Mapped to phases: 234
- Unmapped: 0
- Active satisfied: 218 / 218 (100%)
- Removed (intentional): 15 (see "Removed and Superseded in v1.1" section)
- Superseded by architecture: 1 (DOC-04 → Phase 23)

---
*Requirements defined: 2026-03-15*
*Last updated: 2026-05-04 after milestone audit — marked 15 reqs as removed in stabilization, DOC-04 as superseded by template-refiner*
