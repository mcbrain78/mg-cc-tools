...
Write the concept spec using the template structure from `{MG_INSTALL_CONCEPT_TEMPLATE}`.
...
- `/mg-temp:spec-improve docs/work-queue/todo/{name}/concept.md` — refine with fresh-eyes subagent review
...
- The concept spec template is at `{MG_INSTALL_CONCEPT_TEMPLATE}`. All sections must be present in the output. The LLM judges appropriate depth per section.
