...
---
name: mg-temp:spec-prepare-context
description: Split a multi-phase source document into per-phase files for downstream ingestion by mg-temp:spec-create-context
...
The output files are consumed by `mg-temp:spec-create-context`, which transforms them into GSD CONTEXT.md format.
...
- `1 docs/design/foundation-notes.md` (equivalent to `mg-temp:spec-create-context` input prep)
</context>
...
Usage: /mg-temp:spec-prepare-context <start>-<end> <source-file-path>

Examples:
  /mg-temp:spec-prepare-context 1-5 docs/work-queue/todo/doc-command/DESIGN.md
  /mg-temp:spec-prepare-context 3 docs/design/pipeline-spec.md
```
Exit.
...
- options:
  - "Create all (Recommended)" — "Run mg-temp:spec-create-context for each phase sequentially"
  - "Create specific" — "Let me pick which phases to ingest"
  - "Stop here" — "I'll run mg-temp:spec-create-context manually later"
...
```
```
Skill("mg-temp:spec-create-context", "{phase_number} {source-dir}/phase-docs/phase-{NN}-{slug}.md")
```
...
To ingest manually:
  /mg-temp:spec-create-context 1 {source-dir}/phase-docs/phase-01-{slug}.md
  /mg-temp:spec-create-context 2 {source-dir}/phase-docs/phase-02-{slug}.md
  ...
...
<important_notes>
- This command splits content — it does NOT transform into GSD CONTEXT.md format. That is `mg-temp:spec-create-context`'s job.
