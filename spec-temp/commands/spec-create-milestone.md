...
---
name: mg-temp:spec-create-milestone
...
The deterministic checks are all run by `{MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py`, never eyeballed in-context. This command orchestrates; that script decides.
</objective>
...
Requirements template snapshot: `{MG_INSTALL_REQUIREMENTS_SNAPSHOT}`
Checks script: `{MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py` — always invoked as `uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py <subcommand> [args]` (the family's pinned interpreter prefix). GSD-side inputs live at fixed `.planning/` paths, so only `citations`/`check` take the spec path and `preflight` takes `<version>`.
...
Usage: /mg-temp:spec-create-milestone <version> <spec-path>

Example:
  /mg-temp:spec-create-milestone v6.0 docs/work-queue/todo/point-in-time-l2/concept.md
```
...
Run:
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py preflight <version>
```
...
Run /mg-temp:spec-draft then /mg-temp:spec-improve first.
```
Exit. (Genuinely unstructured docs have the `plan-phase --prd` path at phase level.)

Validate the citation contract (D8):
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py citations <spec-path>
```
...
Run /mg-temp:spec-improve <spec-path> to add the missing citations.
```
...
1. Read the stored snapshot at `{MG_INSTALL_REQUIREMENTS_SNAPSHOT}`.
...
- question: "GSD's requirements template has changed since mg-temp:spec-create-milestone was last updated. {summary of what changed and whether the projection is still compatible}. How should I proceed?"
- options:
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py snapshot --matches
```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py check <spec-path>
```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py research-pre
```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py research-post
```
...
- **Approve** →
  ...
  uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py snapshot
  ```
...
- **Reject** →
  ```bash
  uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py snapshot --clear
  ```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py phase-start
```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py restore
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py coverage
```
...
```bash
uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py traceability
...
## Next Steps
  ...
  /mg-temp:spec-prepare-context {first}-{last} {spec-path}    ← split the spec into per-phase files
  /mg-temp:spec-create-context {N} {per-phase file}           ← per phase: project the split into CONTEXT.md
...
- **The snapshot at `{MG_INSTALL_REQUIREMENTS_SNAPSHOT}` is not updated at runtime on drift** — it persists until the command source is updated and reinstalled, so the drift warning keeps appearing until then.
- **This command requires GSD** (`gsd-tools.cjs` + the agents) — unlike its sibling `spec-create-context`.
- **Always invoke the script as `uv run {MG_INSTALL_SCRIPTS_DIR}/milestone_checks.py <sub>`** — the family's pinned interpreter prefix.
