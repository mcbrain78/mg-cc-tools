...
1. Parse `$ARGUMENTS` to get the target file path. If missing:
   ...
   Usage: /mg-temp:spec-improve <file-path>

   Example:
     /mg-temp:spec-improve docs/work-queue/todo/worktrees/CONCEPT.md
   ```
   Exit.
...
3. **Guarded init.** Initialize the working session by running:
   ...
   - **On guard-fail (exit 1):** an in-progress working copy from a prior session exists. Do **not** overwrite it blindly. Get the resolved paths read-only:
     ...
     - **Discard and restart** — run `uv run {MG_INSTALL_SCRIPTS_DIR}/improve_files.py init <target-file-path> --fresh` (archives the prior session's sidecars, then re-creates a clean working copy from the original). Use its JSON.
...
10. Citation discipline — in `### What gets built`, every top-level bullet (a
    ...
    a GSD milestone via mg-temp:spec-create-milestone.)
...
- `init` is **guarded** (concept D7, inherited from `improve_files.py` and shared with `mg-temp:spec-improve-auto`): it refuses to overwrite an in-progress working copy, prompting resume-or-restart instead of silently discarding a prior session's work. `--fresh` is the explicit discard-and-restart path.
